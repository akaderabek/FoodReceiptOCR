...
FAFHin<-paste("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/02 FAFH Sample Images/Quality 4/", 
             list.files("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/02 FAFH Sample Images/Quality 4/"), sep="") 
# create string with path & output file name
FAFHout<-paste("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/04 FAFH Processed/", 
      list.files("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/02 FAFH Sample Images/"), sep="") 
# standardize file extension case for image_write()
FAFHout<-str_replace(FAFHout, ".JPG", ".jpg") 
FAFHout<-str_replace(FAFHout, ".PNG", ".png") 
...
for (i in 1:8) {
  original<-image_read(FAFHin[i], density = 300) # load original image
  ...
  image_write(deskewed, path=FAFHout[i], density = 300) # write new image file to folder
} 
...
# create string with path & input file name
Crop.in<-paste("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/04 FAFH Processed/",
               list.files("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/04 FAFH Processed/"), sep="")
# create string with path & output file name
Crop.out<-paste("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/04 FAFH Processed/FAFH Cropped/",
                list.files("K:/Research/Umich_West/2020 Receipt Validation/TesseractOCR/04 FAFH Processed/"), sep="")
Crop.in<-Crop.in[1:10]
Crop.out<-Crop.out[1:10]

i<-1
# import pre-processed receipt image, then display in browser and print image properties in console
image<-image_read(Crop.in[i], density = 300); image_browse(image); image

# create cropped image object, then display in browser
cropped<-image_crop(image, geometry = "900x205+450+1400"); image_browse(cropped)
...
image_write(cropped, path = Crop.out[i], density = 300); i<-i+1
